...
describe("Products", () => {
  ...
  describe("/POST product route", () => {
    it("should not POST a product without a required field", (done) => {
      let product = {
        title: "Refrigerator",
        // version: "S56",
        yearMade: 2022,
      ...
      product.save((err, product) => {
        chai
          ...
          .end((err, res) => {
            ...
            res.body.should.have
              .property("message")
              .eql("Product updated succesfully");
            res.body.should.have.nested
              .property("product.title")
              .eql("Television");
            res.body.should.have.nested
      ...
      product.save((err, product) => {
        chai
          ...
          .end((err, res) => {
            ...
            res.body
              .map((e) => e.version)
              .every((version) => version.should.match(/S/i));
            done();
          });
      });
    });
  ...
  describe("/SEARCH/:key product", () => {
    it("should not return product searched by given key when no key matches", (done) => {
      let product = new Product({
        title: "Refrigerator",
        version: "Samsung S56",
        yearMade: 2022,
      ...
      product.save((err, product) => {
        chai
          ...
          .end((err, res) => {
            ...
            res.body
              .map((e) => e.version)
              .every((version) => version.should.match(/K/i));
            done();
          });
      });
    });
